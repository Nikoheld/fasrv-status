...
- Public input is normalized, size limited, rate limited, proof-of-work protected, and checked for high-confidence prompt injection before it reaches a model. Normal technical details, paths, stack traces, and code references are allowed.
...
- Availability and performance recovery is enabled for every configured application. The local watcher checks Docker containers and systemd units, including dependencies and restart counters, and verifies recovery after each action. Content-specific bug remediation remains limited to Jellyfin: playback failures may restart Jellyfin, image failures may request a targeted metadata/image refresh, and failed HiAnime downloads may requeue one conservatively matched failed job.
...
- GitHub issues are processed only when their author is exactly the configured trusted user, currently `Nikoheld`. Issues from every other account are recorded as seen and ignored before their content is evaluated.
- Every processed trusted issue receives one idempotent fixed-template outcome comment. Successful local repairs are closed; declined or failed repairs remain open and explain why no automatic repair was completed.
- High-confidence prompt-injection findings are isolated to the affected report or issue and no longer pause unrelated repairs. A secret finding in generated output still creates `PAUSED` and blocks model calls, remediation, and GitHub writes before anything can be published.
