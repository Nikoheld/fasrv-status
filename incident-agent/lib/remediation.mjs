...
const GENERAL_CRASH_ACTIONS = {
  availability: ["no_action", "restart_origin", "reload_proxy"],
  performance: ["no_action", "restart_origin", "reload_proxy"]
};

const NO_ACTION_TEXT = {
  unsupported_application: "Keine automatische Reparatur ausgeführt: Inhaltliche Bug-Reparaturen sind auf Jellyfin begrenzt.",
...
const SUCCESS_TEXT = {
  restart_origin: (appName) => `${appName} wurde neu gestartet und anschließend erfolgreich geprüft.`,
  reload_proxy: (appName) => `Der Proxy für ${appName} wurde neu geladen und anschließend erfolgreich geprüft.`,
...
export function allowedActionsFor(app, category, hasSeries) {
  ...
  const candidates = app.slug === "jellyfin"
    ? JELLYFIN_ACTIONS[category] ?? ["no_action"]
    : GENERAL_CRASH_ACTIONS[category] ?? ["no_action"];
...
export function noActionReasonFor(app, category, hasSeries) {
  if (app.slug !== "jellyfin" && !Object.hasOwn(GENERAL_CRASH_ACTIONS, category)) return "unsupported_application";
...
export function outcomeComment({ action, incidentId, appName = "Die Anwendung", noActionReason, failureCode }) {
  ...
  const template = SUCCESS_TEXT[action];
  if (!template) throw new Error("unknown_outcome_action");
  const result = typeof template === "function" ? template(appName) : template;
  return `${result}\n\nSolved by: Grok 4.5\n${marker}`;
}
